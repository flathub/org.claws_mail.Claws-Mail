...
Plugins will follow naming convention of `org.claws_mail.Claws.Mail.<PLUGIN_NAME>`. After installation, libraries to load will be available under `/app/extra-plugins/lib` directory.

Inside Flatpak sandbox, plugin will be available under `/app/extra-plugins/<PLUGIN_NAME>`. For convenience, there's `/app/etra-plugins/lib` directory that combines libraries from all plugins. No need to look for plugin shared library in plugin-specific directory - it is all in one place.
